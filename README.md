# Discuss Skill

一个完全自包含的结构化议题研讨技能，基于金字塔原理（SCQA 框架），支持本地、对话、云文档三种持久化方式。通用设计，可运行于支持 skill 机制的任意智能体平台。

## 特性

- **核心功能内置**：议题拆解（MECE）、深度追问、verdict 裁决均已内置，无需依赖其他技能
- **三种持久化模式**：本地文件 / 纯对话 / 云文档（飞书/钉钉/腾讯文档/语雀等），按用户意图自动选择
- **verdict 质量裁决**：每个议题追问后给出 solid / needs-work / invalid 评级，强制对论据质量负责
- **硬性拆解上限**：递归深度 ≤4 层，子议题 ≤5 个，避免无限递归和信息过载
- **通用触发**：支持关键词触发（讨论//discuss）和语义触发，适配不同平台

## 快速开始

### 1. 安装

将 `SKILL.md` 及 `references/`、`assets/` 目录复制到你的智能体技能目录中。

### 2. 触发

支持两种触发方式（因平台而异）：
- **关键词触发**：`讨论` / `讨论下` / `/discuss` / `discuss this` + 话题
- **语义触发**：平台根据 description 自动匹配（如"分析""拆解"复杂问题）

### 3. 使用示例

```
用户：/discuss 分析为什么最近项目延期严重

技能会自动：
1. 检测持久化模式（本地/对话/云端）
2. 按 MECE 拆解议题为 3-5 个子议题
3. 引导逐个讨论，深度追问后给出 verdict
4. 循环直至所有议题 ✅
5. 生成 SCQA 文档 + 落地规划
```

> 完整示例见 `references/examples.md`。

## 文件结构

```
discuss-skill/
├── SKILL.md                 # 主技能文件（核心机制与流程）
├── README.md                # 使用说明（本文件）
├── references/
│   └── examples.md          # 完整讨论流程 + 三模式差异 + verdict 示例
└── assets/
    ├── notes-template.md    # 讨论过程记录模板
    └── summary-template.md  # SCQA 汇总文档模板
```

## 核心机制（详见 SKILL.md）

| 主题 | 说明 |
|------|------|
| **触发方式** | 关键词（讨论//discuss）+ 语义匹配，因平台而异 |
| **拆解规则** | MECE 检查，子议题 3-5 个，递归深度 ≤4 层 |
| **持久化模式** | 本地 / 对话 / 云端（飞书），默认对话模式兜底 |
| **verdict 裁决** | solid / needs-work / invalid，禁止跳过 |
| **SCQA 文档** | 背景-冲突-问题-解决方案 + 落地规划 + 风险应对 |

## 许可证

MIT License
